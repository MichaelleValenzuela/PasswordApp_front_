...
export default defineComponent({
    ...
    mounted() {
        if (Object.keys(this.objectToEdit.data).length === 0) {
            ...
            this.adminResourceStoreStore.form_data.name_type_encrypt = this.objectToEdit.data.name_type_encrypt;
            this.adminResourceStoreStore.form_data.name_resource = this.objectToEdit.data.name_resource;
            this.adminResourceStoreStore.arrMiniDataResource = this.objectToEdit.data.fields;
        }
    },
    watch: {
        isModalOpen() {
                    ...
                    <div class="card_modal px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div style="letter-spacing: 1px; margin-bottom:5%; font-weight: bold; font-size: 1.7em;">
                                {{ Object.keys(objectToEdit.data).length > 0 ? "Editar recurso" : "Agregar recurso" }}
                            </div>
                        </div>
                        <form class="pt-3 pb-3 mb-4">
                            ...
                            <div v-if="Object.keys(objectToEdit.data).length === 0" class="content_inputs_add_resource">
                                <div style="margin-bottom: 5%;">
                            ...
                            <div class="content_data_loaded"
                                v-if="Object.keys(objectToEdit.data).length === 0 && adminResourceStoreStore.arrMiniDataResource.length > 0">
                                <div class="item_data_loaded"
                                    v-for="(r, k) in  adminResourceStoreStore.arrMiniDataResource  " :key="r.label_input">
                                    <div class="btn_actions" v-if="(Object.keys(this.objectToEdit.data).length > 0) && k > 0">
                                    ...
                                    <div class="btn_actions" v-if="(Object.keys(this.objectToEdit.data).length === 0)">
                                        <button type="button" class=" btn_edit_mini"
                                            @click="adminResourceStoreStore.onEditData(r)"
                                            :style="{ color: adminResourceStoreStore.isMiniFormDataEdit ? 'white' : '', pointerEvents: adminResourceStoreStore.isMiniFormDataEdit ? 'none' : '' }">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                    ...
                                    <small
                                        v-if="(Object.keys(this.objectToEdit.data).length > 0) && (r.label_input === 'Nombre' || r.label_input === 'nombre') && k === 0"
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <div style="margin-left: 3%;">
                            <button type="button" class="BTN_SUCCESS_1 py-2 px-4 rounded"
                                @click="adminResourceStoreStore.actionMainAdminResourceStore(Object.keys(objectToEdit.data).length > 0 ? 'ACTION_EDIT_RESOURCE' : 'ACTION_CREATE_NEW_RESOURCE', Object.keys(objectToEdit.data).length > 0 ? objectToEdit.data._id : null, onCloseModal)">
