...
<template>
    ...
    <div v-else-if="authStoreStore.DAPI_RESPONSE.response.status === 404" style="text-align:center; font-size: 3rem; margin-top: 8rem;">        
            {{ authStoreStore.DAPI_RESPONSE.response.msg }}        
    </div>
    <div v-else style="text-align: center;">
        <div v-if="authStoreStore.DAPI_RESPONSE.response.status === 201" style="text-align:center; font-size: 3rem; margin-top: 25%;">
            <div>
                {{ authStoreStore.DAPI_RESPONSE.response.msg }}
            </div>
        </div>
